...
def main():
    ...
    if options.male_only:
        keep_patients = albany_meta_df.loc[(albany_meta_df['COVID'] == 1) & (albany_meta_df['ICU_1']) & (albany_meta_df['Gender'] == 'M')].index
    else:
        keep_patients = albany_meta_df.loc[(albany_meta_df['COVID'] == 1) & (albany_meta_df['ICU_1'])].index
    ...
    # TODO REMOVE?????? This is the gray colormap...
    norm = mpl.colors.Normalize(-1,1)
    colors = [[norm(-1.0), "#BFBFBF"],
          [norm( 1.0), "black"]]
    grays = mpl.colors.LinearSegmentedColormap.from_list("", colors)
    #################################################
